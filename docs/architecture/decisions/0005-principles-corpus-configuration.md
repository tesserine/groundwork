...
Forge addresses are runtime-owned session identity delivered as the structured
`RUNA_FORGE_ADDRESSES` payload. The corpus is methodology content resolved
once at setup, not per-session identity. Env vars also offer no structural
validation; the repo's established
