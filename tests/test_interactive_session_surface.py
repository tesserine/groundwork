...
class InteractiveSessionSurfaceTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory(prefix="groundwork-runa-go-") as tmp:
            ...
            write_executable(
                ...
                {{
                    ...
                    printf '%s\\n' '{{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{{"name":"contract","arguments":{{"instance_id":"contract-1","title":"Interactive sessions use the session surface","criteria":[{{"id":"session-surface-records-output","lens":"behavior","acceptance_criterion":"Interactive sessions reach define through the runa session surface","statement":"The scoped session records the produced contract artifact through the MCP session surface.","hollow_delivery":"The agent prints a contract but no validated artifact is persisted.","kind":"behavior","check":{{"actor":"interactive session smoke test","procedure":"Run the interactive session smoke test against the MCP session surface.","observable":"The produced contract artifact is persisted and validates.","conforming_case":"The recorded artifact reads back schema-valid.","falsifying_case":"The agent prints a contract but no validated artifact is persisted."}}}}]}}}}}}'
    ...
    def test_session_sequences_documentation_deliverable_gate_form_behavior(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="groundwork-runa-gate-form-") as tmp:
            ...
            workspace = project_dir / ".runa" / "workspace"
            (workspace / "work-unit").mkdir(parents=True)
            criterion = "A runtime-backed documentation-deliverable work-unit delivers its behavior lens without scenarios."
            (workspace / "work-unit" / f"{work_unit_id}.json").write_text(
                json.dumps(
                    {
            ...
            write_executable(
                ...
                {{
                    ...
                    case "$protocol" in
                      define)
                        printf '%s\\n' '{{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{{"name":"contract","arguments":{{"instance_id":"contract-1","title":"Lens criteria stay on the contract surface","criteria":[{{"id":"gate-form-behavior-artifacts","lens":"behavior","acceptance_criterion":{json.dumps(criterion)},"statement":"Gate-form behavior artifacts stay schema-valid through the runtime MCP tool.","hollow_delivery":"The artifact validates only by fabricating a scenario-shaped contract.","kind":"behavior","check":{{"actor":"runtime MCP validation tool","procedure":"Validate the gate-form artifact through the MCP tool.","observable":"The artifact validates against the contract schema.","conforming_case":"The gate-form artifact is accepted by the tool.","falsifying_case":"The artifact validates only by fabricating a scenario-shaped contract."}}}}]}}}}}}'
                        ;;
                      plan)
                        printf '%s\\n' '{{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{{"name":"implementation-plan","arguments":{{"instance_id":"plan-1","summary":"Map gate-checked behavior through the uniform criterion mapping.","design_decisions":[{{"decision":"Key the mapping by contract criterion id.","rationale":"The contract criterion is the single cross-stage traceability key for every lens."}}],"affected_files":["schemas/contract.schema.json"],"criterion_mapping":[{{"criterion_id":"gate-form-behavior-artifacts","steps":["Validate the gate artifact.","Advance to implement on the produced artifact."]}}]}}}}}}'
                        ;;
                      implement)
