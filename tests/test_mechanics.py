...
class MechanicTests(unittest.TestCase):
    ...
    def test_schema_rejects_deployment_value_parameter_metadata(self) -> None:
        mechanic = {
            "name": "sourcehut-upload",
        ...
        self.assertIn("parameters/0", context.exception.paths)

    def test_schema_rejects_secret_deployment_value_parameter_shape(self) -> None:
        mechanic = {
            "name": "sourcehut-upload",
            "purpose": "Upload to a configured forge repository.",
            "default_invocation": 'printf "%s\\n" "$repo_number"',
    ...
    def test_schema_rejects_forge_tag_with_field_path(self) -> None:
        with self.assertRaises(MechanicError) as context:
