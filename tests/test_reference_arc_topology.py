...
class ReferenceArcTopologyTests(unittest.TestCase):
    ...
    def test_forge_operations_are_connector_capability_operations_not_provider_mechanics(self) -> None:
        manifest_data = manifest()
        mechanics = {entry["name"]: entry for entry in manifest_data["mechanics"]}

        for operation in operation_names():
            with self.subTest(operation=operation):
                self.assertIn(operation, mechanics)
                self.assertNotIn("forge_tags", mechanics[operation])

        self.assertNotIn("forge_tags", manifest_data)
    ...
    def test_land_approved_proposal_resolution_uses_work_unit_and_version_together(self) -> None:
        ...
        self.assertEqual(v1["handle"]["display"], "proposal 340 v1")
        self.assertEqual(v2["handle"]["display"], "proposal 340 v2")
