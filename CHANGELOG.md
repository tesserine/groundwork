...
- Submit-review-land disposition workflow migration: source `submit` and
  `land` contracts, manifest routing through `change-proposal`,
  `change-approved`, and `change-needs-revision`, revision re-activation of
  submit, WHAT-layer protocol prose for submit and land, and retirement of the
  live `patch` artifact type and fixtures (closes #340; supersedes #330 and
  #332).
