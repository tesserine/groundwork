...
rendered command text. Forge operations are not C-3 mechanics in Groundwork;
they are connector capability operations declared in `manifest.toml`.
...
connector-issued `handle`, while non-tracker units omit `handle`, and
planning-phase work-unit bodies do not carry a top-level `work_unit` field.

`forge-capability-v1.schema.json` is a vendored copy of the canonical forge
capability contract maintained by `tesserine/commons`.
`forge-capability-v1.provenance.json` records the immutable source URL and
semver. Groundwork uses that vendored schema as the authority for forge
operation names and the handle shape. Artifact schemas inline the same
`{ id, display }` handle definition so validators can remain self-contained;
conformance verifies those projections against the vendored contract.
