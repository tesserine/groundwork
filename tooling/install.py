...
- **The methodology runtime** — ``~/.groundwork`` with the manifest and the
  vendored connector capability contracts the runtime uses to validate
...
def project_runtime_bundle(source: Path, sha: str, target: Path) -> None:
    ...
    (target / "schemas" / "forge-capability-v1.schema.json").write_bytes(
        show_file(source, sha, "schemas/forge-capability-v1.schema.json")
    )
    (target / "schemas" / "forge-capability-v1.provenance.json").write_bytes(
        show_file(source, sha, "schemas/forge-capability-v1.provenance.json")
    )
...
RUNTIME_BUNDLE_CHILDREN = ("manifest.toml", "schemas", MARKER_NAME)
OBSOLETE_RUNTIME_BUNDLE_CHILDREN = ("mechanics", "lib", "bin")
...
def converge_runtime_bundle(options: Options, sha: str) -> None:
    ...
    try:
        ...
        for child in OBSOLETE_RUNTIME_BUNDLE_CHILDREN:
            current = runtime_root / child
            if current.is_dir():
                shutil.rmtree(current)
            elif current.exists():
                current.unlink()
        if runtime_root.is_dir() and tree_payload(staging, include_marker=True) == bundle_payload(runtime_root):
            return
